...
var I18N = {
  ...
  de: {
    ...
    footer: { our_loans:'Unsere Kredite', business:'Unternehmen', info:'Informationen', about:'Ueber uns', legal:'Impressum', privacy:'Datenschutz', cgu:'AGB', contact:'Kontakt', copyright:'2025 B-Mo Financial. Alle Rechte vorbehalten.', license:'ACPR-Zulassung Nr. 123456', brand_desc:'Zugelassene Privatgesellschaft. Aktiengesellschaft mit einem Kapital von 10.000.000 EUR.', brand_text:'Zugelassene Privatgesellschaft. Aktiengesellschaft mit einem Kapital von 10.000.000 EUR. Sitz: Friedrichstrasse 100, 10117 Berlin.' },
    testimonials: { title:'Was unsere Kunden sagen', subtitle:'Ueber 12.000 zufriedene Kunden vertrauen uns', t1_text:'Dank B-Mo Financial konnte ich mein Traumhaus kaufen. Der Prozess war einfach, schnell und vollstaendig online. Ich habe meine Antwort innerhalb von 24 Stunden erhalten!', t1_name:'Maria K.', t1_detail:'Immobilienkredit · 280.000 EUR', t2_text:'Als Selbststaendiger hatte ich Muehe, einen Kredit zu bekommen. B-Mo Financial hat meinen Antrag in 2 Tagen bearbeitet. Sehr professionelles Team!', t2_name:'Thomas H.', t2_detail:'Betriebskredit · 45.000 EUR', t3_text:'Der Online-Antrag war sehr einfach. Ich konnte meinen Dossier in Echtzeit verfolgen. Die Beraterin war sehr zuhoerend und kompetent.', t3_name:'Sophie L.', t3_detail:'Privatkredit · 18.000 EUR' },
    team: { title:'Unser Team', subtitle:'Experten, die Ihnen bei jedem Schritt helfen', sophie_role:'Leiterin Privatkundschaft', sophie_exp:'12 Jahre Erfahrung', marc_role:'Immobilienfinanzierung', marc_exp:'8 Jahre Erfahrung', anna_role:'Unternehmensberaterin', anna_exp:'10 Jahre Erfahrung', paul_role:'Mikrofinanzierung', paul_exp:'6 Jahre Erfahrung' },
    contact: { title:'Kontakt & Standort', subtitle:'Unser Team steht Ihnen von Montag bis Freitag von 8:00 bis 18:00 Uhr zur Verfuegung.', hq_label:'Hauptsitz', hq_value:'Friedrichstrasse 100\n10117 Berlin, Deutschland', phone_label:'Telefon', email_label:'E-Mail', hours_label:'Oeffnungszeiten', hours_value:'Mo–Fr: 08:00 – 18:00 Uhr\nSa: 09:00 – 13:00 Uhr', form_title:'Schnelle Nachricht senden', name_label:'Ihr Name', name_placeholder:'Max Mustermann', email_placeholder:'max@email.de', msg_label:'Ihre Nachricht', msg_placeholder:'Ich interessiere mich fuer...', send_btn:'Nachricht senden', toast:'Nachricht gesendet! Wir melden uns bald.' },
  ...
  document.querySelectorAll('[data-i18n]').forEach(function(el) {
    ...
    if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
      ...
      /* Élément parent avec enfants traduits — ne toucher qu'au premier nœud texte direct */
  ...
  updateSelectorUI: function() {
    ...
    var btnMobile = document.getElementById('lang-current-mobile');
    var all = {}; var k;
    ...
    if (info) {
      if (btn) btn.textContent = info.flag + ' ' + info.label;
      if (btnMobile) btnMobile.textContent = info.flag + ' ' + info.label;
    }
  ...
  renderSelector: function() {
    ...
    var containerMobile = document.getElementById('lang-selector-mobile');
    ...
    if (containerMobile) {
      var htmlMobile = '<div class="lang-wrapper"><button id="lang-current-mobile" class="lang-btn" onclick="I18N.toggleDropdown()">🇩🇪 Deutsch</button><div id="lang-dropdown-mobile" class="lang-dropdown"><div class="lang-group-label">Traductions complètes</div>';
      for (code in this.supported) { var lm = this.supported[code]; htmlMobile += '<button class="lang-option" onclick="I18N.setLang(\'' + code + '\');I18N.closeDropdown()" data-lang="' + code + '">' + lm.flag + ' ' + lm.label + '</button>'; }
      htmlMobile += '<div class="lang-group-label">Via Google Translate</div>';
      for (code in this.googleOnly) { var lgm = this.googleOnly[code]; htmlMobile += '<button class="lang-option" onclick="I18N.setLang(\'' + code + '\');I18N.closeDropdown()" data-lang="' + code + '">' + lgm.flag + ' ' + lgm.label + '</button>'; }
      htmlMobile += '</div></div>';
      containerMobile.innerHTML = htmlMobile;
      document.addEventListener('click', function(e) { if (!containerMobile.contains(e.target)) { var ddm = document.getElementById('lang-dropdown-mobile'); if (ddm) { ddm.style.display = 'none'; } } });
    }
  },

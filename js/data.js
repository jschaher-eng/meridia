...
async function loadClients() {
  ...
  CLIENTS = (data || []).map(u => ({
    ...
    phone: u.phone || '—', city: u.city || '—', status: u.status || 'active',
    score: u.credit_score || 0, income: u.monthly_income || 0,
    created: formatDate(u.created_at), loans: 0,
    avatar: initials(u.full_name || u.email), color: 'navy',
  }));
}
...
async function loadLoans() {
 ...
 LOANS = (data || []).map(l => ({
  ...
  CLIENTS.forEach(c => { c.loans = LOANS.filter(l => l.clientId === c.id).length; });
}

async function loadMessages() {
  const { data, error } = await supabase.from('messages').select('*, sender:users!from_id ( full_name, email ), loan:loans ( reference )').order('created_at', { ascending: false });
  if (error) { console.error('loadMessages:', error.message); return; }
  const convMap = {};
  (data || []).forEach(m => {
    const key = (m.from_id || '') + '_' + (m.loan_id || '');
    if (!convMap[key]) {
      const senderName = m.sender?.full_name || m.sender?.email || 'Client';
      convMap[key] = { id:key, clientName:senderName, clientAvatar:initials(senderName), loanRef:m.loan?.reference||'—', unread:0, lastTime:formatTime(m.created_at), messages:[] };
    }
    if (!m.read) convMap[key].unread++;
    convMap[key].messages.push({ recv:true, by:m.sender?.full_name||'Client', at:formatTime(m.created_at), text:m.content||'' });
  });
  MSG_CONVERSATIONS = convMap;
}

async function loadDocuments() {
  const { data, error } = await supabase.from('documents').select('*, users ( full_name ), loans ( reference )').order('created_at', { ascending: false });
  if (error) { console.error('loadDocuments:', error.message); return; }
  DOCUMENTS = (data || []).map(d => ({
    id:d.id, clientId:d.user_id, client:d.users?.full_name||'—', loan:d.loans?.reference||'—',
    name:d.name||'Document', type:d.type||'identite', size:d.size||'—',
    date:formatDate(d.created_at), status:d.status||'pending', ext:d.ext||'PDF',
  }));
}
...
const STATUS_LABEL = { pending:'Antrag eingereicht', reviewing:'Dokumentenpruefung', analysis:'Aktenanalyse', approved:'Grundsatzentscheidung', fees:'Gebuehrenzahlung', signed:'Vertragsunterzeichnung', funded:'Auszahlung der Mittel', active:'In Rueckzahlung', rejected:'Abgelehnt', suspended:'Suspendu' };
const STATUS_BADGE = { pending:'b-warn', reviewing:'b-info', analysis:'b-info', approved:'b-ok', fees:'b-warn', signed:'b-ok', funded:'b-ok', active:'b-ok', rejected:'b-err', suspended:'b-err' };
const TYPE_ICONS = { contrat:'#185FA5', immo:'#0F6E56', identite:'#854F0B' };
